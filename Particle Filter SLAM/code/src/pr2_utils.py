...
def compute_stereo(path_l, path_r, data_root, filename, disp=False):
    '''
    I have modified this function to create disparity images that are used for 
    texture mapping and write the filenames into a CSV file as time stamps.
    '''
    if disp:
    ...
    if disp:
        ...
        isExist = os.path.exists(data_root+"/disparity/")
        if not isExist:
          os.makedirs(data_root+"/disparity/")
          
        cv2.imwrite(data_root+"/disparity/"+filename, disparity)

        with open(data_root+'/cam_time.csv', 'a', newline='') as f_object:  
        # Pass the CSV  file object to the writer() function
            writer_object = writer(f_object)
            # Result - a writer object
            # Pass the data in the list as an argument into the writerow() function
            writer_object.writerow([str(filename.split(".")[0])])  
...
def read_data_from_csv(filename):
    ...
    '''
    #Make sure to change this, adding nrows param to help fasten the process during implentation
    data_csv = pd.read_csv(filename, nrows=10000, header=None)
...
def make_time_equal(enc_data, fog_data):
    enc_time = enc_data[0]
    enc_data = enc_data[1]
    fog_time = fog_data[0]
    fog_data = fog_data[1]
    #print(fog_data)
    fog = np.zeros(np.size(enc_time))
    for i in range(np.size(enc_time)):
        diff = abs(fog_time - enc_time[i])
        index1 = np.argmin(diff)
        diff[index1] = 10000
        index2 = np.argmin(diff)
        fog[i] = (fog_data[index1]+fog_data[index2])/2
    
    return fog

def create_disparity_map(data_root):
    '''
    This function takes in the stereo images path as input and creates disparity images
    that are later used for texture mapping
    Took the approach of saving the disparity images first and then using it rather than
    generating it on the fly to avoid system calls for reading the files and calculating
    image disparities
    '''
    data_root_left = data_root+"stereo_left"
    data_root_right = data_root+"stereo_right"
    for filename in os.listdir(data_root_left):
        #Assuming both the folders have images with the same name
        img_left = os.path.join(data_root_left, filename)
        if filename in os.listdir(data_root_right):
            img_right = os.path.join(data_root_right, filename) 
            compute_stereo(img_left, img_right, data_root, filename)

if __name__ == '__main__':
    #Modified compute_stereo function for debugging
    #compute_stereo(None, None, None, None,disp=True)
    create_disparity_map("../../data/stereo_images/")
   # show_lidar()
    #test_mapCorrelation()
    #test_bresenham2D()
